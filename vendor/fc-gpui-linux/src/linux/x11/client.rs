...
struct PointerDeviceState {
    ...
    /// When the last scroll wheel dispatch for this device happened.
    ///
    /// Obelisk vendored patch: the X server reports one physical wheel tick
    /// as EITHER an emulated button press (detail 4/5/6/7), a smooth
    /// valuator motion event, or BOTH (in either order, on real X11 with
    /// libinput). XWayland (e.g. Hyprland/GNOME Wayland hosting X11 apps)
    /// delivers ONLY the emulated button press — the upstream code skipped
    /// those unconditionally, making the wheel completely dead there.
    /// Both kinds are dispatched now, deduplicated by this timestamp.
    last_scroll_dispatch: Option<Instant>,
}

impl PointerDeviceState {
    /// Returns true when this wheel tick should be dispatched: the first
    /// scroll within a short window wins, later ones (the duplicate half of
    /// the same physical tick reported through the other event kind) are
    /// skipped. 30ms is far below the fastest human wheel repetition.
    fn should_dispatch_scroll(&mut self) -> bool {
        let now = Instant::now();
        if self
            .last_scroll_dispatch
            .is_some_and(|at| now.duration_since(at) < Duration::from_millis(30))
        {
            false
        } else {
            self.last_scroll_dispatch = Some(now);
            true
        }
    }
}
...
pub struct X11ClientState {
    ...
    pub(crate) tray: crate::linux::tray::LinuxTray,
    pub(crate) global_hotkey: crate::linux::global_hotkey::x11::X11GlobalHotkey,
}
    ...
    fn handle_event(&self, event: Event) -> Option<()> {
        match event {
            ...
            Event::XkbStateNotify(event) => {
                ...
                let modifiers = crate::linux::modifiers_from_xkb(&state.xkb);
                let capslock = crate::linux::capslock_from_xkb(&state.xkb);
            ...
            Event::KeyPress(event) => {
                ...
                let keystroke = {
                    ...
                    let mut keystroke =
                        crate::linux::keystroke_from_xkb(&state.xkb, modifiers, code);
                    ...
                    if let Some(mut compose_state) = state.compose_state.take() {
                        ...
                        match compose_state.status() {
                            ...
                            xkbc::Status::Cancelled => {
                                ...
                                if let Some(current_key) = crate::linux::underlying_dead_key(keysym)
                                {
            ...
            Event::KeyRelease(event) => {
                ...
                let keystroke = {
                    let code = event.detail.into();
                    let keystroke = crate::linux::keystroke_from_xkb(&state.xkb, modifiers, code);
                    let keysym = state.xkb.key_get_one_sym(code);
            ...
            Event::XinputButtonPress(event) => {
                ...
                match button_or_scroll_from_event_detail(event.detail) {
                    ...
                    Some(ButtonOrScroll::Scroll(direction)) => {
                        // Obelisk vendored patch: emulated scroll buttons must be
                        // dispatched too. XWayland delivers the wheel ONLY as
                        // emulated button presses (no smooth valuator motion
                        // ever follows), so skipping them — as upstream did —
                        // makes scrolling completely dead there. The dispatch
                        // is deduplicated against a simultaneous smooth scroll
                        // (real X11 with libinput reports both) via
                        // `should_dispatch_scroll`.
                        let dispatch = state
                            .pointer_device_states
                            .get_mut(&event.sourceid)
                            .map_or(true, |pointer| pointer.should_dispatch_scroll());
                        drop(state);
                        if dispatch {
            ...
            Event::XinputMotion(event) => {
                ...
                if let Some(mut pointer) = state.pointer_device_states.get_mut(&event.sourceid) {
                    ...
                    // Obelisk vendored patch: dedupe against the emulated
                    // button press of the same physical tick (which may have
                    // been dispatched already — the X server can deliver the
                    // pair in either order).
                    let dispatch =
                        scroll_delta.is_some() && pointer.should_dispatch_scroll();
                    drop(state);
                    if dispatch {
                        let scroll_delta = scroll_delta.unwrap();
    ...
    fn xim_handle_event(&self, event: Event) -> Option<()> {
        match event {
            Event::KeyPress(event) | Event::KeyRelease(event) => {
                let mut state = self.0.borrow_mut();
                state.pre_key_char_down = Some(crate::linux::keystroke_from_xkb(
                    &state.xkb,
                    state.modifiers,
                    event.detail.into(),
                ));
...
impl LinuxClient for X11Client {
    ...
    fn display(&self, id: DisplayId) -> Option<Rc<dyn PlatformDisplay>> {
        ...
        Some(Rc::new(
            X11Display::new(&state.xcb_connection, state.scale_factor, id.index()).ok()?,
        ))
    }
    ...
    fn focused_window_info(&self) -> Option<gpui::FocusedWindowInfo> {
        let state = self.0.borrow();
        let root = state.xcb_connection.setup().roots[state.x_root_index].root;
    ...
    fn register_global_hotkey(&self, id: u32, keystroke: &Keystroke) -> gpui::Result<()> {
        let mut state = self.0.borrow_mut();
    ...
    fn request_user_attention(&self, _level: gpui::AttentionType, handle: Option<AnyWindowHandle>) {
        let state = self.0.borrow();
    ...
    fn get_cursor_icon(&mut self, style: CursorStyle) -> Option<xproto::Cursor> {
        ...
        match style {
            ...
            _ => 'outer: {
                ...
                let cursor_icon_names = crate::linux::cursor_icon_names(style);
